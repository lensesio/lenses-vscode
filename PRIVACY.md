...
For telemetry aggregation, the Extension uses `vscode.env.machineId` — an anonymous, stable identifier generated by VS Code. This is a SHA-256 hash that **cannot be traced back to you or your machine**. It is used solely to de-duplicate sessions and measure active installations. It is never combined with personally identifiable information.

### Infrastructure Name Hashing

When telemetry events reference Kafka infrastructure names (e.g., environment names, topic names), these values are one-way hashed (SHA-256, truncated) before transmission. The original names cannot be recovered from the hashes.
...
- **Infrastructure Names**: Environment and topic names are hashed before transmission
